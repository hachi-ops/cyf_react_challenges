...
export default function Jobs(props) {
  ...
  const lang = languages.map((langButton, index) => {
    return (
      <button
        className="languages"
        onClick={() => props.addFilterArr(langButton)}
      >
        {langButton}
      </button>
    );
  ...
  const toolsMap = tools.map((toolButton, index) => {
    return (
      <button className="tools" onClick={() => props.addFilterArr(toolButton)}>
        {toolButton}
      </button>
    );
  });
      ...
      <div className="jobPost">
        <section className="jobInfo">
          ...
          <section className="companyDetails">
            ...
            <p className="details">
              {props.listings.postedAt} &#8226; {props.listings.contract}{" "}
              &#8226; {props.listings.location}
            </p>
          </section>
          ...
          <div className="buttonDiv">
            <button
              className="role"
              onClick={() => props.addFilter(props.listings.role)}
            >
              {props.listings.role}
            </button>
            <button
              className="level "
              onClick={() => props.addFilter(props.listings.level)}
            >
              {props.listings.level}
            </button>
            {lang}
            {toolsMap}
          </div>
